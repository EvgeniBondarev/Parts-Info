        ...
        <table className="min-w-full table-fixed">
          ...
          <thead>
            <tr className="border-b border-gray-200">
              {titles.map((title, key) => (
                <th key={key} className="px-2 py-3 text-center text-xl font-bold text-gray-500 bg-gray-50 w-1/3">
                  {title}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-gray-200">
            {data.slice(0, visibleRows).map((row, index) => (
              ...
              >
                {row.map((spec, key) => (
                  <td key={key} className="px-2 py-3 text-xl text-gray-700 font-medium text-center truncate w-1/3">
                  {spec || 'Не указано'}
                </td>
                ))}
              </tr>
      ...
      {data.length > rowLimit && (
        <div className="p-1 border-t border-gray-200">
          <button
            ...
            className="w-full text-blue-600 hover:text-blue-800 font-bold text-lg flex items-center justify-center gap-2"
          >
            <svg
